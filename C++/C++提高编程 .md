...
总结：主流的解决方式是第二种，将类模板成员函数写到一起，并将后缀名改为.hpp  //类模板！！！就这么搞
...
void test01() {
	...
	vector<int>::iterator pBegin = v.begin();//迭代器相当于指针，vector<int>::interator相当于数据类型
	vector<int>::iterator pEnd = v.end();

	//第一种遍历方式：
	while (pBegin != pEnd) {//链表
		cout << *pBegin << endl;
		pBegin++;
	}
	...
	for_each(v.begin(), v.end(), MyPrint);//回调技术，起始迭代器，结束迭代器，函数指针调用
}
...
void test01() {
	...
	for (vector<Person>::iterator it = v.begin(); it != v.end(); it++) {
		cout << "Name:" << (*it).mName << " Age:" << (*it).mAge << endl;//迭代器相当于指针
...
[(44条消息) STL vector的N种构造方式_vector构造_伟哥爱编程的博客-CSDN博客](https://blog.csdn.net/xi_niuniu/article/details/47060043?ops_request_misc=&request_id=&biz_id=102&utm_term=vector有参构造&utm_medium=distribute.pc_search_result.none-task-blog-2~all~sobaiduweb~default-2-47060043.142^v88^control_2,239^v2^insert_chatgpt&spm=1018.2226.3001.4187)
